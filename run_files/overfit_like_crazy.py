...
# Fit model
recmodel.fit(tensor_provider=the_tensor_provider,
          train_idx=train_idx,
          verbose=2, batch_size=BATCH_SIZE, n_batches=ITERS)


# Predict on train-data
y_pred_train = recmodel.predict(tensor_provider=the_tensor_provider,
                       predict_idx=train_idx)
y_pred_train = np.squeeze(y_pred_train)


# Predict on test-data for performance
y_pred_test = recmodel.predict(tensor_provider=the_tensor_provider,
...
for evaluation_nr, evalf in enumerate(eval_functions):
    ...
    evaluation_results_train = evalf(y_true_train, y_pred_train)
    ...
    evaluation_results_test = evalf(y_true_test, y_pred_test)
